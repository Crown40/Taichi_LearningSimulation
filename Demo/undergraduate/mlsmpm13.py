...
n_particles, n_grid = 8*16384,128 #8*16384,512  #6000,128 #9000 * ,128 * quality #quality**2, 128 * quality
dx, inv_dx = space_length / n_grid, float(n_grid)
dt = 1e-6 / quality #1e-5
...
# p
xi=12.0 #10.0
critical_compression,critical_stretch=2.5e-2,7.5e-3 #2.5e-2,4.5e-3 
# g
g_acc=1e6 #1e3  #9.8
vCO=ti.Vector([0,]*dim)
# collision grid
  ...
  for I in ti.grouped(grid_m):
    if grid_m[I] > 0: # No need for epsilon here
      ...
      for c in ti.static(range(dim)):
        ...
        if I[c] > n_grid - 3 and vG[c] > 0: vG = [0,]*dim
      ...
      #for c in ti.static(range(dim)):
      #  if I[c] < 3 and vG[c] < 0:          vG[c] = 0
      #  if I[c] > n_grid - 3 and vG[c] > 0: vG[c] = 0
...
def initialize():
  ...
  #  for d in ti.static(range(4)):
        ...
        x[i]=ti.Vector([0.3+0.4*ti.random(),0.4+0.2*ti.random(),0.425+0.15*ti.random()])*space_length
...
file_name='demo\\snowCube_p\\collision\\collision_p.ply'
